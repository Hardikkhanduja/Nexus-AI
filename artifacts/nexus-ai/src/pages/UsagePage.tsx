...
export default function UsagePage({ onOpenSidebar }: UsagePageProps) {
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col relative overflow-hidden bg-[#0D0D12]" data-testid="page-usage">
      {/* HEADER */}
      <header className="h-16 flex items-center justify-between px-6 border-b border-[#FF4FD8]/20 bg-[#0D0D12]/80 backdrop-blur-sm z-10 shrink-0">
        <div className="flex items-center gap-4">
          <button
            onClick={onOpenSidebar}
            className="p-1 text-[#FF4FD8] hover:bg-[#FF4FD8]/10 transition-colors rounded-sm"
            data-testid="button-open-sidebar-usage"
          >
            <div className="flex flex-col gap-[3px] w-5">
              <div className="h-[2px] bg-[#FF4FD8] w-full shadow-[0_0_4px_currentColor]"></div>
              <div className="h-[2px] bg-[#FF4FD8] w-full shadow-[0_0_4px_currentColor]"></div>
              <div className="h-[2px] bg-[#FF4FD8] w-full shadow-[0_0_4px_currentColor]"></div>
            </div>
          </button>
          <h2 className="font-display text-xs text-[#FF4FD8] text-shadow-primary tracking-wider mt-1">USAGE & LIMITS</h2>
        </div>
      </header>
